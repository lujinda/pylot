...
class commaneHandler:
    ...
    def unknow(self,session,cmd):
        ...
        publicMethods = map(lambda funcname:funcname[3:], publicMethods)
...
class Room(commaneHandler):
    ...
    def do_logout(self,session,line):
        ...
        session.d.errback(failure.Failure)

class LoginRoom(Room):
    def add(self,session):
        Room.add(self,session)
        print session.session.getPeer()
...
class ChatRoom(Room):
    ...
    def remove(self,session):
        ...
        mess=session.name+" has left the room"
        self.broadcast(mess)
